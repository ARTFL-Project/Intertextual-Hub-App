...
diff_match_patch.prototype.diff_main = function(a, b, c, d) {
    ...
    c && a.unshift([DIFF_EQUAL, c]);
    g && a.push([DIFF_EQUAL, g]);
...
diff_match_patch.prototype.diff_compute_ = function(a, b, c, d) {
    ...
    return -1 != g ?
        ((c = [
                [DIFF_INSERT, e.substring(0, g)],
                [DIFF_EQUAL, f],
                [DIFF_INSERT, e.substring(g + f.length)],
            ]),
...
diff_match_patch.prototype.diff_bisect_ = function(a, b, c) {
    ...
    k = d - e;
    for (
        var m = 0 != k % 2, p = 0, x = 0, w = 0, q = 0, t = 0; t < f && !(new Date().getTime() > c); t++
    ) {
        ...
        for (v = -t + w; v <= t - q; v += 2) {
            ...
            u =
                ...
                l[n - 1] + 1;
            for (
                r = u - v; u < d && r < e && a.charAt(d - u - 1) == b.charAt(e - r - 1);
...
diff_match_patch.prototype.diff_cleanupSemantic = function(a) {
    for (
        var b = !1, c = [], d = 0, e = null, f = 0, g = 0, h = 0, l = 0, k = 0; f < a.length;
...
diff_match_patch.prototype.diff_cleanupSemanticLossless = function(a) {
    ...
    for (var c = 1; c < a.length - 1;) {
        if (a[c - 1][0] == DIFF_EQUAL && a[c + 1][0] == DIFF_EQUAL) {
            ...
            h = e;
            for (
                var l = f, k = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0);
            ...
            case DIFF_EQUAL:
                1 < c + d ?
                    (0 !== c &&
                        ...
                        0 === c ?
                        a.splice(b - d, c + d, [DIFF_INSERT, f]) :
                        0 === d ?
                        a.splice(b - c, c + d, [DIFF_DELETE, e]) :
                        a.splice(
                            b - c - d,
                            c + d, [DIFF_DELETE, e], [DIFF_INSERT, f]
                        ),
...
diff_match_patch.prototype.diff_fromDelta = function(a, b) {
    for (
        var c = [], d = 0, e = 0, f = b.split(/\t/g), g = 0; g < f.length; g++
    ) {
...
diff_match_patch.prototype.match_bitap_ = function(a, b, c) {
    ...
    1 != h &&
        ((g = Math.min(d(0, h), g)),
            (h = a.lastIndexOf(b, c + b.length)), -1 != h && (g = Math.min(d(0, h), g)));
    var l = 1 << (b.length - 1);
    h = -1;
...
diff_match_patch.prototype.patch_addContext_ = function(a, b) {
    if (0 != b.length) {
        for (
            var c = b.substring(a.start2, a.start2 + a.length1), d = 0; b.indexOf(c) != b.lastIndexOf(c) &&
            c.length <
        ...
        a.diffs.unshift([DIFF_EQUAL, c]);
        (d = b.substring(a.start2 + a.length1, a.start2 + a.length1 + d)) &&
        a.diffs.push([DIFF_EQUAL, d]);
        a.start1 -= c.length;
        a.start2 -= c.length;
            ...
            for (l = 0; l < a[f].diffs.length; l++) {
                ...
                p[0] === DIFF_INSERT ?
                    ...
                    (b =
                        b.substring(0, k + m) +
                        b.substring(
                            k + this.diff_xIndex(g, h + p[1].length)
                        ));
...
diff_match_patch.prototype.patch_addPadding = function(a) {
    ...
    0 == e.length || e[e.length - 1][0] != DIFF_EQUAL ?
        (e.push([DIFF_EQUAL, c]), (d.length1 += b), (d.length2 += b)) :
...
diff_match_patch.prototype.patch_splitMax = function(a) {
    for (var b = this.Match_MaxBits, c = 0; c < a.length; c++)
        if (!(a[c].length1 <= b)) {
            ...
            a.splice(c--, 1);
            for (
                var e = d.start1, f = d.start2, g = ""; 0 !== d.diffs.length;
...
diff_match_patch.prototype.patch_fromText = function(a) {
    ...
    for (
        ...
        for (c++; c < a.length;) {
            ...
            if ("-" == e) f.diffs.push([DIFF_DELETE, g]);
            else if ("+" == e) f.diffs.push([DIFF_INSERT, g]);
            else if (" " == e) f.diffs.push([DIFF_EQUAL, g]);
            else if ("@" == e) break;
            else if ("" !== e)
